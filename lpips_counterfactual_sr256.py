#!/usr/bin/env python
"""LPIPS pair-wise counterfactual for sr256 (baseline + variant a).

sr256 has 31 synthesis layers; the last stem layer (boundary) is detected
automatically as the last entry of layer_names that is NOT in
head_layer_names.
"""
...
BATCH = int(os.environ.get('BATCH', '4'))   # sr256 is bigger; smaller batch
...
stem_layers = [n for n in layer_names if n not in head_set]
BOUNDARY = stem_layers[-1]
print(f'  z_dim={z_dim}  c_dim={c_dim}')
print(f'  total layers = {len(layer_names)}, stem = {len(stem_layers)}, head = {len(head_set)}')
print(f'  boundary (last stem layer) = {BOUNDARY}')
...
print(f'\nRunning {N_SAMPLES} samples, batch={BATCH}')
...
while n_done < N_SAMPLES:
    ...
    z_list = []
    for i in range(bs):
        torch.manual_seed(n_done + i)
        z_list.append(torch.randn(1, z_dim))
    z_batch = torch.cat(z_list, dim=0).to(device)
...
print('\n=== LPIPS pair-wise (sr256 counterfactual) ===')
...
out_json = os.path.join(REPO, 'lpips_counterfactual_sr256.json')
with open(out_json, 'w') as f:
    json.dump({'samples': N_SAMPLES, 'boundary': BOUNDARY,
               'per_variant': summary}, f, indent=2)
print(f'\nWrote {out_json}')
