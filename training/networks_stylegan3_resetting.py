...
class SuperresGenerator(torch.nn.Module):
    def __init__(
        ...
        **synthesis_kwargs,
        ...
        # Older public pkls (e.g. official StyleGAN-XL ImageNet stems) do not store
        # `conv_kernel` / `use_radial_filters` as instance attrs on SynthesisNetwork;
        # fall back to layer-level conv_kernel (max over non-toRGB layers) and the
        # stylegan3-t default of use_radial_filters=False.
        if hasattr(G_stem.synthesis, 'conv_kernel'):
            self.conv_kernel = G_stem.synthesis.conv_kernel
        else:
            layer_kernels = [
                getattr(G_stem.synthesis, n).conv_kernel
                for n in G_stem.synthesis.layer_names
                if not getattr(G_stem.synthesis, n).is_torgb
            ]
            self.conv_kernel = max(layer_kernels) if layer_kernels else 3
