...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        force_label_dim = None, # Override the inferred label dimension. Use when the data is a subset of
                                # a wider label space (e.g. an ImageNet-100 subset paired with a 1000-class stem).
    ):
        self._name = name
        ...
        self._force_label_dim = force_label_dim
    ...
    def label_shape(self):
        if self._label_shape is None:
            ...
            if raw_labels.dtype == np.int64:
                ...
                if self._force_label_dim is not None:
                    assert self._force_label_dim >= inferred, (
                        f"force_label_dim={self._force_label_dim} is smaller than the max-label-derived "
                        f"size {inferred}; would lose classes")
                    self._label_shape = [int(self._force_label_dim)]
                else:
                    self._label_shape = [inferred]
            else:
