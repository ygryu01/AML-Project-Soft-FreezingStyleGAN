...
def training_loop(
    ...
    soft_freeze             = False,    # Variant B/C: train all stem layers but at reduced LR. Splits G optimizer into two parameter groups.
    stem_lr_mult            = 0.1,      # Variant B/C: stem-group LR multiplier (initial value when schedule != 'const').
    stem_lr_schedule        = 'const',  # Variant D: 'const' | 'linear' | 'cosine'. Decay shape for stem-group LR from stem_lr_mult to 0.
    stem_lr_decay_kimg      = 0,        # Variant D: kimg at which stem LR reaches 0. 0 = use total_kimg.
):
    # Initialize.
    ...
    conv2d_gradfix.enabled = False                      # Disabled: cudnn_convolution_backward_weight removed in PyTorch 1.11+
    grid_sample_gradfix.enabled = False                 # Disabled: grid_sampler_2d_backward signature changed in PyTorch 1.11+
    ...
    def _stem_lr_mult_eff(cur_kimg):
        """Variant D: schedule-aware multiplier for the stem param group.
        Starts at stem_lr_mult, decays to 0 by stem_lr_decay_kimg (or total_kimg)."""
        decay = float(stem_lr_decay_kimg) if stem_lr_decay_kimg and stem_lr_decay_kimg > 0 else float(total_kimg)
        if stem_lr_schedule == 'const' or decay <= 0:
            return float(stem_lr_mult)
        t = min(max(cur_kimg / decay, 0.0), 1.0)
        if stem_lr_schedule == 'linear':
            return float(stem_lr_mult) * (1.0 - t)
        if stem_lr_schedule == 'cosine':
            return float(stem_lr_mult) * 0.5 * (1.0 + math.cos(math.pi * t))
        return float(stem_lr_mult)

    def _split_g_params(g_module, head_layer_names):
        """Variant B/C helper: split G parameters into (head, stem) lists.
        Stem includes mapping and any synthesis layer not named in head_layer_names."""
        head_param_ids = set()
        for layer_name in head_layer_names:
            for p in getattr(g_module.synthesis, layer_name).parameters():
                head_param_ids.add(id(p))
        head_params, stem_params = [], []
        for p in g_module.parameters():
            (head_params if id(p) in head_param_ids else stem_params).append(p)
        return head_params, stem_params

    # Variant D: list of (opt, head_base_lr) pairs whose stem (param_groups[1]) LR
    # should be retuned each tick when stem_lr_schedule != 'const'.
    stem_scheduled_opts = []

    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
        # Variant B/C: build a parameter-group list with stem at lr * stem_lr_mult.
        if name == 'G' and soft_freeze and hasattr(module, 'head_layer_names'):
            head_p, stem_p = _split_g_params(module, module.head_layer_names)
            base_lr = float(opt_kwargs.lr)
            params_arg = [
                {'params': head_p, 'lr': base_lr},
                {'params': stem_p, 'lr': base_lr * float(stem_lr_mult)},
            ]
            if rank == 0:
                print(f"[soft_freeze] G optimizer: head_lr={base_lr:.5f} (n={len(head_p)} tensors), "
                      f"stem_lr={base_lr * stem_lr_mult:.5f} (n={len(stem_p)} tensors)")
        else:
            params_arg = list(module.parameters())

        if reg_interval is None:
            opt = dnnlib.util.construct_class_by_name(params=params_arg, **opt_kwargs) # subclass of torch.optim.Optimizer
            if name == 'G' and soft_freeze and hasattr(module, 'head_layer_names'):
                stem_scheduled_opts.append((opt, float(opt_kwargs.lr)))
            ...
            if name == 'G' and soft_freeze and hasattr(module, 'head_layer_names'):
                ...
                params_arg = list(module.parameters())
            opt = dnnlib.util.construct_class_by_name(params_arg, **opt_kwargs) # subclass of torch.optim.Optimizer
            if name == 'G' and soft_freeze and hasattr(module, 'head_layer_names'):
                stem_scheduled_opts.append((opt, float(opt_kwargs.lr)))
            for _ in range(iter_dict[name]):
    ...
    while True:
        ...
        # Variant D: update stem param-group LR per schedule.
        if soft_freeze and stem_lr_schedule != 'const' and len(stem_scheduled_opts) > 0:
            mult_eff = _stem_lr_mult_eff(cur_nimg / 1000.0)
            for opt, head_base_lr in stem_scheduled_opts:
                opt.param_groups[1]['lr'] = head_base_lr * mult_eff
            training_stats.report0('Progress/stem_lr_mult', mult_eff)
            if rank == 0:
                print(f"[stem_lr_schedule:{stem_lr_schedule}] kimg={cur_nimg/1000:.1f} stem_lr_mult={mult_eff:.4f}")
