...
# Soft-freeze variants for super-res training
@click.option('--unfreeze_last_stem_layers', help='Variant A: also unfreeze the last N stem synthesis layers during super-res. 0 = head-only baseline.', type=click.IntRange(min=0), default=0, show_default=True)
@click.option('--soft_freeze', help='Variant B/C: train all stem layers (no freezing) but at a reduced LR via --stem_lr_mult.', is_flag=True)
@click.option('--stem_lr_mult', help='Variant B/C: stem LR multiplier relative to head LR. Only used with --soft_freeze.', type=click.FloatRange(min=0), default=0.1, show_default=True)
@click.option('--preserve_weight', help='Variant C: weight on the stem-preservation loss term. 0 disables it.', type=click.FloatRange(min=0), default=0.0, show_default=True)
@click.option('--preserve_metric', help='Distance metric for the stem-preservation loss.', type=click.Choice(['mse', 'l1', 'lpips']), default='mse', show_default=True)
@click.option('--label_dim',    help='Override dataset label dimension (one-hot size). Useful when the dataset is a subset of a wider label space (e.g. ImageNet-100 paired with a 1000-class stem). 0 = auto-infer.', type=click.IntRange(min=0), default=0, show_default=True)
@click.option('--stem_lr_schedule', help='Variant D: decay shape for the stem-group LR. const = constant (= variant B/C). linear/cosine = decay stem_lr_mult to 0 by --stem_lr_decay_kimg.', type=click.Choice(['const', 'linear', 'cosine']), default='const', show_default=True)
@click.option('--stem_lr_decay_kimg', help='Variant D: kimg at which stem LR reaches 0. 0 = use total kimg.', type=click.IntRange(min=0), default=0, show_default=True)
...
def main(**kwargs):
    ...
    if opts.label_dim > 0:
        c.training_set_kwargs.force_label_dim = int(opts.label_dim)
    ...
    if opts.superres:
        ...
        # Soft-freeze variants override the default head-only freezing.
        # Variant A (--unfreeze_last_stem_layers N): keep mapping frozen and
        # everything except the last N stem synthesis layers frozen.
        # Variant B (--soft_freeze, --stem_lr_mult m): no layer is frozen, but
        # stem parameters get a separate optimizer group with LR = G_lr * m.
        # Variant C (--soft_freeze plus --preserve_weight w): like Variant B
        # plus a stem-output-preservation penalty in ProjectedGANLoss.
        c.loss_kwargs.train_head_only = not opts.soft_freeze
        c.loss_kwargs.unfreeze_last_stem_layers = int(opts.unfreeze_last_stem_layers)
        c.loss_kwargs.preserve_weight = float(opts.preserve_weight)
        c.loss_kwargs.preserve_path_stem = opts.path_stem if opts.preserve_weight > 0 else None
        c.loss_kwargs.preserve_metric = str(opts.preserve_metric)
        c.soft_freeze = bool(opts.soft_freeze)
        c.stem_lr_mult = float(opts.stem_lr_mult)
        c.stem_lr_schedule = str(opts.stem_lr_schedule)
        c.stem_lr_decay_kimg = int(opts.stem_lr_decay_kimg)
